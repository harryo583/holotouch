"""
This script contains a class to control the screen brightness based on hand gestures using Mediapipe and OpenCV.
...
class BrightnessController:
    @log_call
    def __call__(self, frame, landmarks):
        """
        ...
        cv2.putText(frame, f"Brightness level: {brightness_level}%", (40, height - 40),
                    cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 255, 255), 5, cv2.LINE_AA)
        brightness = int(brightness_level / 10) / 10
        with run_subprocess(["brightness", str(brightness)]):
            pass
